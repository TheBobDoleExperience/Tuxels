...
## 30. PSD Import (read-only) (M12)

- `src/io/PsdIO.{h,cpp}` exposes `loadPsd(path)` and `loadPsdBytes(span)`. The byte-buffer entry point keeps unit tests clean — they synthesize PSD fixtures programmatically rather than committing binaries.
- **Scope:** 8-bit-per-channel RGB color mode (mode == 3, depth == 8). Other depths (1, 16, 32) and color modes (Bitmap, Grayscale, Indexed, CMYK, Multichannel, Duotone, Lab) rejected with explicit error messages. Compression 0 (raw) and 1 (PackBits RLE) supported; ZIP variants (2, 3) rejected. PSB (large-document) format rejected — PSD only.
- **Cursor.** Big-endian byte reader that fails fast on truncation. `failed_` propagates so post-fail reads are no-ops; the caller checks `cur.ok()` at convenient boundaries instead of branching every read. PSD's nested length-prefixed sections are handled via `cur.seek(blockEnd)` after each block — defensive jumps past unconsumed bytes so a misinterpreted sub-block doesn't desynchronise the rest of the parse.
- **PackBits decoder.** Per Apple's spec: control byte n in [0..127] = literal copy of next n+1 bytes; n in [-127..-1] = repeat next byte (1-n) times; n == -128 = no-op. Per-row decode so a bad row's overrun stops at the row boundary instead of trashing subsequent rows. The PSD spec provides a per-row byte-count table preceding the RLE payload; we skip it because PackBits is self-delimiting given a known target output size.
- **Layer record extra-data.** After the fixed-shape layer header, an `extraDataLen u32` block contains: u32 mask-data length + mask metadata; u32 blending-ranges length + N bytes (skipped); a Pascal-padded layer name (1 byte length + bytes + zero pad to a multiple of 4); then a stream of additional info blocks each starting with `8BIM` (or `8B64`) signature + 4-byte key + u32 length + payload + 2-byte alignment pad. The reader scans for `lsct` (section divider) and `luni` (Unicode name override); other blocks are skipped via the per-block length.
- **Section dividers (group nesting).** PSD brackets a group's contents bottom-to-top: a type-3 record at the bottom and a type-1 (open) or type-2 (closed) folder-header record at the top carrying the group's actual name + blend + opacity + visibility. The reader runs a parse stack: type-3 pushes a fresh GroupLayer (placeholder name) into the current parent and onto the stack; intermediate records append to the most-recent open group via the stack; type-1/2 finalizes the group with the header record's metadata and pops. Pure bottom-to-top order — the same parse-stack pattern used for `.txl` v5 (§14 v5 bullet).
- **Masks.** Per-layer user mask channel (id == -2) is decoded into a single-byte buffer sized to the mask's separate (top, left, bottom, right) rect, then expanded to a `LayerMask` carrying `Rgba32F{v,v,v,1}` per pixel (compose's mask sampler reads only `.r`). Real (vector) masks (id == -3) are skipped.
- **Blend keys.** norm/diss/mul/scrn/over/dark/lite/idiv/lddg/sLit/hLit/diff/smud/pass map to the local enum. PS uses both 4-char codes and 3-char codes padded with space (e.g. "mul "). Unknown keys fall back to Normal so the layer at least appears.
- **Defensive boundaries.** Each section (color mode data, image resources, layer & mask info, layer info subsection, per-layer extra data) is bracketed by reading its u32 length, recording the start position, then `cur.seek(start + len)` at the end. Misaligned reads don't propagate.
- **What's missing.** Adjustment-layer records, smart objects, text layers, layer effects (`lfx2`), clipping flags, real (vector) masks. Records of those types are read for their layer-record skeleton (so the parse doesn't drift) and emit a plain `PixelLayer` carrying whatever composite contribution is in the channel data (often the rasterized form PS embeds). Future work picks each up individually.
