...
- **S6 completed**: PNG I/O + File menu wired. `src/io/PngIO.{h,cpp}` uses `QImageReader`/`QImageWriter` through `QImage::Format_RGBA8888` (no sRGB linearization for M0 — passthrough). Extracted into a separate `tuxels_io` STATIC library depending on `Qt6::Gui` so tests can link it without pulling in Widgets or MOC. `File → Open…` wraps the loaded PNG in a fresh `Document` with a single PixelLayer named after the file; `File → Export As PNG…` composes the current document into a `TuxImage` and writes sRGB 8-bit PNG. New `tests/test_png_io.cpp` (round-trip, out-of-range clamp, missing-file error, empty-image error) — 4 tests — now at 47 total passing.
