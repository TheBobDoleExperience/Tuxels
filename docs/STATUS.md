...
| S3 — Free Transform | ⬜ pending | Ctrl+T modal; bbox handles for scale + rotate + translate; bilinear `Resample`; Shift locks aspect / snaps rotation to 15°; Enter commits via `beginRecord/setPixel/stopRecord` + `PaintCommand` |
| S4 — Lasso + Polygonal Lasso | ⬜ pending | L / Shift-L; freehand + click-vertex paths; shared `SelectionMask::fillPolygon` scanline rasterizer; combine-mode options row |
| S5 — Select by Color | ⬜ pending | Shift-W cycles from Magic Wand; non-contiguous tolerance pick across entire active layer |
| S6 — Brush dynamics | ⬜ pending | Size jitter, opacity jitter, spacing sliders; deterministic per-stroke RNG seed |
| S7 — Verify + tag `v0.2.0-m2` | ⬜ pending | User walkthrough through the full feature set; round-trip through `.txl`; tag |
