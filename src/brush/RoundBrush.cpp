...
void RoundBrush::rebuildKernel() {
  const int D = p_.diameter;
  kernel_.assign(static_cast<std::size_t>(D) * static_cast<std::size_t>(D), 0.f);
  ...
  for (int y = 0; y < D; ++y) {
    for (int x = 0; x < D; ++x) {
      ...
      if (r >= 1.f) {
        k = 0.f;
      } else if (p_.hardness >= 0.999f || r <= p_.hardness) {
        k = 1.f;
      } else {
        const float t = (r - p_.hardness) / (1.f - p_.hardness);
        k = 1.f - t * t * (3.f - 2.f * t);  // inverse smoothstep
      }
      kernel_[static_cast<std::size_t>(y) * static_cast<std::size_t>(D) + x] = k;
    }
  }
}
